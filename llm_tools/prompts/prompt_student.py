...
A machine learning model predicted that a student will FAIL their final exams and therefore their academic progress was flagged as at risk.
...
3. STUDENT PROFILE: The student's specific feature values with comparisons to dataset averages
...
- Which factors were most important in this decision
- How their specific situation compared to typical students
- What changes would be needed to flip the prediction to "will pass" and remove the academic risk flag

INFORMATION YOU WILL RECEIVE:
1. DATASET INFORMATION: Context about the dataset, target variable and ML task used to train the model
2. COUNTERFACTUAL EXPLANATION: Information about what counterfactuals are and how to interpret them
3. STUDENT PROFILE: The student's specific feature values with comparisons to dataset averages and the model's predicted probability of failure
...
In other words: Each counterfactual is a "what if" scenario showing the minimum feature changes needed to flip the model's prediction from "will fail" to "will pass". This helps answer: "What would need to be different for the academic risk flag to be removed?"

Study habits, attendance, family support, and personal circumstances can vary. The counterfactuals show which combinations of changes would be most effective in improving academic success prospects.
"""
...
def build_cf_prompt(instance_index, cf_csv_path: str = None, adverse_csv_path: str = None, shap_csv_path: str = None) -> str:
    ...
    Parameters:
    - instance_index: the instance index to explain (e.g., 10, 25, etc.)
    - cf_csv_path: path to counterfactual CSV (defaults to student_dataset/student_counterfactual.csv)
    - adverse_csv_path: path to adverse CSV (defaults to student_dataset/student_adverse.csv)
    - shap_csv_path: path to SHAP CSV for predicted_probability (defaults to student_dataset/student_shap.csv)
    
    Returns:
    - Full prompt string ready for LLM
    """
    if cf_csv_path is None:
        cf_csv_path = Path(__file__).parent.parent.parent / "datasets_prep" / "data" / "student_dataset" / "student_counterfactual.csv"
...
if __name__ == "__main__":
    ...
    # Example instance indices from student_adverse.csv
    example_indices = [1]
