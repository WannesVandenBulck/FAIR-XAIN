...
3. APPLICANT PROFILE: The applicant's specific feature values with comparisons to dataset averages and distributions
...
3. APPLICANT PROFILE: The applicant's specific feature values with comparisons to dataset averages and distributions, and the model's predicted probability of failure
...
def build_cf_prompt(instance_index, cf_csv_path: str = None, adverse_csv_path: str = None, shap_csv_path: str = None) -> str:
    ...
    feature_cols = [col for col in adverse_df.columns 
                   if col not in ['instance_index', 'original_test_index', 'predicted_class', 'prediction_score', 'actual_target', 'target_credit']]
